...
export default function AddItemModal({
  ...
  const errorPopup = (
    <div className="fixed inset-0 z-60 flex justify-center pt-12 items-start bg-black/20" style={{backdropFilter:"blur(2px)"}}>
      <div className="relative max-w-xs w-full bg-[var(--custom-accent-red)] border-2 border-[var(--custom-cream-yellow)] rounded-md shadow-xl p-5 animate-popup-in animate-border-pulse">
        <button
  ...
  const modalGlass = "bg-gradient-to-br from-[var(--custom-bg-secondary)] to-[var(--custom-bg-primary)] bg-opacity-70 rounded-md shadow-2xl border-2 border-[var(--custom-ocean-blue)]";
    ...
    >
      ...
      >
        <button
          onClick={onClose}
          disabled={!!error}
          className="absolute right-6 top-6 bg-[var(--custom-accent-red)] text-white hover:bg-[var(--custom-cream-yellow)] hover:text-[var(--custom-accent-red)] shadow-lg rounded-full w-9 h-9 flex items-center justify-center transition-colors duration-200"
          aria-label="close"
        >×</button>
        ...
        <div
          className={`relative mb-4 min-h-[150px] rounded-md border-2 border-dashed transition-all duration-300 ${glowingBorder} bg-[var(--custom-bg-tertiary)] flex flex-col items-center justify-center group cursor-pointer select-none shadow-inner`}
              ...
              {images.length > 4 ?
                  ...
                  <div className="flex flex-row items-center w-full justify-between animate-fadeIn overflow-hidden max-w-full" style={{maxHeight: "60vh"}}>
                    ...
                    <div className="flex-1 flex items-center justify-center overflow-hidden">
                      <img
                        ...
                        className="w-full h-full max-h-[56vh] max-w-full mx-2 rounded-md border-2 border-[var(--custom-ocean-blue)] shadow-lg animate-fadeIn object-contain"
                      />
                    </div>
                    <button
                      className="bg-[var(--custom-ocean-blue)]/90 hover:bg-[var(--custom-cream-yellow)] text-white hover:text-[var(--custom-ocean-blue)] rounded-full p-2 shadow transition-all duration-200 z-10"
                ...
                <div className="flex gap-4 flex-wrap items-center justify-center w-full animate-fadeIn">
                  {images.map((img, idx) => (
                    <div
                      key={idx}
                      className="relative inline-block rounded-md shadow ring-2 ring-[var(--custom-ocean-blue)] transition-all duration-300 hover:scale-110 hover:ring-[var(--custom-cream-yellow)]"
                      style={{ display: 'inline-block' }}
                    >
                      <img
                        src={URL.createObjectURL(img)}
                        alt="Preview"
                        className="object-contain max-h-36 max-w-60 rounded-md transition-all duration-300"
                      />
                      <button
                        type="button"
        ...
        <div className="w-full flex flex-col gap-4">
          <div className="relative mt-1">
            <input
              type="text"
              id="itemName"
              className="peer w-full px-4 pt-4 h-12 text-lg text-[var(--custom-text-primary)] rounded-md bg-[var(--custom-bg-tertiary)] border border-[var(--custom-ocean-blue)] focus:ring-2 focus:ring-[var(--custom-ocean-blue)] outline-none transition"
              value={itemName}
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              <input
                ...
                id="minBid"
                min={0}
                className="peer h-12 px-4 pt-4 w-full text-lg rounded-md bg-[var(--custom-bg-tertiary)] text-[var(--custom-text-primary)] border border-[var(--custom-ocean-blue)] focus:ring-2 focus:ring-[var(--custom-ocean-blue)] outline-none transition"
                value={minBid}
